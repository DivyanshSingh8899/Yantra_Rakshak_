...
### 2026-07-11 — Committed frontend UI/UX changes; deployed frontend to Vercel; fixed Private Network Access CORS bug
- **Files modified**: committed the frontend UI/UX overhaul from the entry above (was previously only working-tree changes) plus a `frontend/dist/` gitignore entry and removal of a stray unused `.claude/launch.json`; `backend/app/main.py` and `simulation/main.py` (`allow_private_network=True` added to each app's `CORSMiddleware`).
- **Summary**: Committed and pushed the pending frontend changes to `origin/main` (user-confirmed). Deployed `frontend/` to Vercel via an already-authenticated local CLI session (project `dss-projects-6d20e6dd/frontend`), live at `https://frontend-two-steel-35.vercel.app` — a UI/design-only deploy, since the app still calls `localhost:8000`/`:8001` for its API/WS/simulation-control endpoints and there is no public backend host. User then reported the Simulation Control Panel showing "Failed to fetch" on that deployed URL. Root cause: the FastAPI backend's and simulation control API's `CORSMiddleware` were rejecting Chrome's Private Network Access preflight (`Access-Control-Request-Private-Network: true`) with `400 Disallowed CORS private-network`, since the installed Starlette version (1.3.1) supports this via an `allow_private_network` flag that wasn't set. Verified via direct `curl` OPTIONS preflight before and after the fix. Both services were killed and restarted with the fix; re-verified the preflight now returns `200` with `Access-Control-Allow-Private-Network: true` on both `:8000` and `:8001`.
- **Reason**: User asked to deploy through Vercel, then reported a real bug from that deployment (not a data-availability issue, which had already been called out and accepted) — root-caused and fixed rather than working around it, since it also affects any future public frontend calling this backend from a different origin, not just this specific deploy.
- **Caveat**: this makes the *browser preflight* succeed for public-origin-to-localhost calls; it does not make the backend reachable from any machine other than the one it's running on. The deployed Vercel site only shows real data when viewed from this same machine while the backend/broker/simulation are running locally — a different visitor's browser would target their own localhost, not this one.
